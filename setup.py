...
class build_ext_subclass( build_ext ):
    ...
    def set_cxxstd(self):
        arg_cxx11 = "-std=c++11"
        if self.test_supports_compile_arg(arg_cxx11):
            for e in self.extensions:
                e.extra_compile_args.append(arg_cxx11)
        else:
            print("C++11 standard not supported. Package might fail to compile.")
    ...
    def test_supports_compile_arg(self, comm, with_omp=False, with_c_comp=False):
        ...
        try:
            ...
            fname = "multinomial_probit_compiler_testing.cpp"
...
setup(
    name  = "multinomial_probit",
    packages = ["multinomial_probit"],
    version = '0.0.1',
    description = 'Multinomial Probit Regression with Approximate Gradients',
    author = 'David Cortes',
    url = 'https://github.com/david-cortes/multinomial_probit',
    keywords = ['mnp', 'multinomial-probit'],
    cmdclass = {'build_ext': build_ext_subclass},
    ext_modules = [Extension(
                            "multinomial_probit._cpp_wrapper",
                            sources=["multinomial_probit/cpp_wrapper.pyx",
                                     "src/univariate.cpp",
                                     "src/bhat.cpp",
                                     "src/tvbs.cpp",
                                     "src/ldl.cpp",
